...
export function generateMndaMarkdown(data: MndaFormData): string {
  const mndaTermText =
...
| :--- | :--- | :--- |
| **Company** | ${data.party1Company || "[Party 1 Company]"} | ${data.party2Company || "[Party 2 Company]"} |
| **Print Name** | ${data.party1Name || "[Party 1 Name]"} | ${data.party2Name || "[Party 2 Name]"} |
| **Title** | ${data.party1Title || "[Party 1 Title]"} | ${data.party2Title || "[Party 2 Title]"} |
| **Notice Address** | ${data.party1Address || "[Party 1 Address]"} | ${data.party2Address || "[Party 2 Address]"} |
| **Date** | ${data.effectiveDate || "[Date]"} | ${data.effectiveDate || "[Date]"} |
